...
const InsightsSection = () => {
  ...
  useEffect(() => {
    const loadExistingSession = async () => {
      ...
      try {
        const response = await processorApi.getChatSession(sessionId);
  ...
  const fetchSuggestedPrompts = useCallback(async () => {
    ...
    try {
      const response = await processorApi.getSuggestedPrompts(
        selectedDocumentIds,
  ...
  }, [selectedDocumentIds, chatSession?.session_id]);
  ...
  }, [selectedDocumentIds, fetchSuggestedPrompts]);
  ...
  const handleSendMessage = async (query?: string) => {
    ...
    setResponseLoading(true);
    try {
      const response = await processorApi.chatWithDocuments({
        session_id: chatSession?.session_id,
        invoice_ids: selectedDocumentIds,
        prompt: messageText,
      });
          ...
          <div className="w-full text-center">
            <div className="flex items-center border border-[#D9D9D9] rounded-full px-4 py-2 shadow-sm mt-5">
              ...
              <Button
                ...
                disabled={responseLoading || selectedDocumentIds.length === 0}
              />
            </div>
