...
const ViewDuplicates = () => {
  ...
  const selectedCount = useMemo(() => {
    return Object.values(selectedDocumentIds).flat().length;
  }, [selectedDocumentIds]);
  ...
  const checkFullySelectedGroups = () => {
    for (const [hash, selectedIds] of Object.entries(selectedDocumentIds)) {
      const totalDocuments =
        duplicateMapByFileHash?.[hash]?.documents?.length || 0;

      if (totalDocuments > 0 && selectedIds.length === totalDocuments) {
  ...
  const pageRefresh = () => {
    setDuplicatesMapByFileHash((prev) => {
      ...
      Object.keys(selectedDocumentIds).forEach((hash) => {
        if (updatedMap[hash]) {
          updatedMap[hash].documents = updatedMap[hash].documents.filter(
            (document) => !selectedDocumentIds[hash].includes(document.id)
          );

          if (updatedMap[hash].documents.length <= 1) {
            ...
            <div className="flex flex-wrap mt-5 mb-5 justify-between gap-5">
              <div className="flex flex-col gap-1">
                <p className="font-medium text-[16px]">
                  List of Duplicate Documents Found
                </p>
                <p>
                  Here are documents with duplicates. You can review and select
