  ...
  const handleIgnore = async () => {
    try {
      ...
      showNotification(
        "success",
        selectedCount > 1
          ? "Selected documents have been successfully ignored"
          : "Selected document has been successfully ignored"
      );
    } catch (error) {
      handleError(error);
    } finally {
    ...
    >
      {selectedCount > 1 ? (
        <p className="text-dark-gray text-sm font-normal ml-[38px] mb-6">
          If you ignore these duplicates, they will remain in the document list
          and won’t be flagged again.
        </p>
      ) : (
        <p className="text-dark-gray text-sm font-normal ml-[38px] mb-6">
          If you ignore this duplicate, it will remain in the document list and
